...
public class Mailinator {
	...
	private static final String MAILINATOR_INBOX_TEMPLATE_URL = MAILINATOR_API_ENDPOINT
			+ "/inbox?token=%s&to=%s&private_domain=%b";
	private static final String MAILINATOR_EMAIL_TEMPLATE_URL = MAILINATOR_API_ENDPOINT
			+ "/email?token=%s&msgid=%s&private_domain=%b";
	private static final String MAILINATOR_DELETE_TEMPLATE_URL = MAILINATOR_API_ENDPOINT
			+ "/delete?token=%s&msgid=%s&private_domain=%b";
	 ...
	 *            - Email address of the account
	 * @param privateDomain
	 *            - indicates if the request is to an account with a private
	 *            domain
	...
	public static List<InboxMessage> getInboxMessages(String apikey, String emailAddress, boolean privateDomain)
		...
		JSONArray jsonMessages = (JSONArray) obj.get("messages"); // messages
																	// array
		...
		if (jsonMessages != null) {
			...
			// The messages come in the response in a reversed order, so lets
			// give
	 ...
	 *            - Mailinator API key
	 * @param emailId-
	 *            ID of email to get, can be retrieved using the
	...
	/**
	 * Once you have the email id's from a given inbox query, you can retrieve
	 * the full email. You can use either a public or a private Mailinator
	 * domain.
	 *
	 * @param apikey
	 *            - Mailinator API key
	...
	private static InboxMessage createInboxMessageFrom(JSONObject jsonInboxMsg) {
		...
		message.setTo(asString(jsonInboxMsg.get("to")));
		message.setId(asString(jsonInboxMsg.get("id")));
		...
		message.setSubject(asString(jsonInboxMsg.get("subject")));
		message.setFromfull(asString(jsonInboxMsg.get("fromfull")));
		message.setFrom(asString(jsonInboxMsg.get("from")));
		message.setBeen_read(asBoolean(jsonInboxMsg.get("been_read")));
		message.setIp(asString(jsonInboxMsg.get("ip")));
